...
namespace InfSystemWebApplication.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                case SignInStatus.RequiresVerification:
                    return RedirectToAction("SendCode", new { ReturnUrl = returnUrl, RememberMe = false /*model.RememberMe*/ });
        ...
        #region Мои вспомогательные приложения
        private async Task AddRole(ApplicationUser user)
        {
            ...
            {
                employee = db.Employees.Find(user.EmployeeId);
            }

            if (employee.PositionId == (int)PositionId.ADMIN)
            {
                await UserManager.AddToRoleAsync(user.Id, "favored_user");
                await UserManager.AddToRoleAsync(user.Id, "admin");
            }
            if (employee.PositionId == (int)PositionId.SENIOR_ASSISTANT)
            {
                await UserManager.AddToRoleAsync(user.Id, "favored_user");
            }
        }
