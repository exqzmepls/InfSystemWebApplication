...
namespace InfSystemWebApplication.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        private string GetUserView(string email)
        {
            int employeeId;

            using (ApplicationDbContext applicationDb = new ApplicationDbContext())
            {
                employeeId = applicationDb.Users.FirstOrDefault(u => u.Email == email).EmployeeId;
            }

            if (employeeId == 0) return "default admin";

            using (InfSystemContext context = new InfSystemContext())
            {
                Employee employee = context.Employees.Find(employeeId);
                context.Entry(employee).Reference(e => e.Person).Load();
                return employee.Person.View;
            }
        }
    }
}